...
**URL** — это сокращение от **Uniform / Universal Resource Locator**, переводящихся как уникальный адрес 
веб-документа в сети интернет. В последнее время система URL-адресов является общепринятой и универсальной при обозначении 
расположения файлов, открытых для пользователей Всемирной паутины.

###  URL и URI

В современном мире **URL** хоть и остается общепринятым, но постепенно уступает **URI**, являющемуся более широким определением 
**Uniform/Universal Resource Identifier**. Ключевое их различие заключается в том, что URI может не обязательно указывать место, 
где располагается объект. Ему присваивается уникальный идентификатор. Это позволяет применять к абсолютно любому объекту URI, 
даже тому, который может быть недоступен в электронном виде. Если смотреть глубже, можно заметить, что URL — это разновидность URL.
