...
# END my limit
```
`max_execution_time` – конфигурация тайм-аута, это время, необходимое для запуска команд и выполнения скриптов. Необходимо 
...
предотвратить загрузку сервера плохо написанными скриптами. настройка по умолчанию: 30. Apache имеет Timeoutдиректива и IIS
имеют тайм-аут CGI функция. Оба значения по умолчанию 300 секунд (5 минут). При запуске PHP из команды строка настройка 
